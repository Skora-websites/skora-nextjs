...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  switch (action) {
    case "register": {
      ...
      const existingUser = await hrmUsersService.findOneInTenant("default", "email", email.toLowerCase());
      ...
      if (db) {
        await db.collection("employee_onboarding_tasks").insertOne({
          ...
          email: email.toLowerCase(), department: body.department || "", documentName: body.documentName || "",
      ...
      const response = NextResponse.json({ data: { uid: newUser.id, email: email.toLowerCase(), displayName: displayName || firstName || email, role } }, { status: 201 });
    ...
    case "reset-password": {
      ...
      // Never log or return the raw reset token. Delivery must happen through a trusted email provider.
    ...
    case "create-user": {
      ...
      const passwordHash = await bcrypt.hash(password, 12);
      const newUser = await hrmUsersService.create({
        email: normalizedEmail, displayName: displayName || normalizedEmail, role: requestedRole,
        status: "active", loginStatus: "enabled", passwordHash, tenantId: auth.tenantId, mustChangePassword: true,
      } as any);
...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  switch (action) {
    ...
    case "user": {
      ...
      if ((user as any).tenantId !== auth.tenantId) return notFound("User not found");
...
export const PATCH = withErrorHandler(async (request: NextRequest) => {
  ...
  if (loginStatus) {
    ...
    if (targetRole === "super_admin" && auth.role !== "super_admin") {
      return forbidden("Only Super Admin can manage a Super Admin account");
    }
    if (auth.role !== "super_admin" && ROLE_HIERARCHY[auth.role] <= ROLE_HIERARCHY[targetRole]) {
      return forbidden("You cannot manage an account at or above your role");
    }
