...
/**
 * Offer letter PDF generation — shared by:
 *  - GET /api/hrm/v2/offer-letters/download (employee/CEO download)
 *  - PATCH /api/hrm/v2/offer-letters (auto-attach to the release email)
 *
 * Render pipeline:
...
export interface OfferLetterPdfConfig {
  companyName?: string;
...
export interface GeneratedOfferLetterPdf {
  buffer: Buffer;
  filename: string;
  password: string;
}
...
export async function generateOfferLetterPdf(
  ...
  cfg: OfferLetterPdfConfig
): Promise<GeneratedOfferLetterPdf> {
  ...
  const refNo = `REF/${new Date(letter.createdAt || Date.now()).getFullYear()}/${letterId.slice(-8).toUpperCase()}`;
  ...
  const filename = `offer-letter-${letter.employeeName.replace(/\s+/g, "-")}.pdf`;
  return { buffer: Buffer.from(encryptedBytes), filename, password };
}
