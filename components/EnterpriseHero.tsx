...
export const EnterpriseBackground = () => (
  <div className="absolute inset-0 z-0 overflow-hidden bg-[#071a42]" aria-hidden="true">
    ...
    <motion.svg viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice" animate={{ x: [0, -24, 0], y: [0, 12, 0], opacity: [0.05, 0.27, 0.05], filter: ["blur(4px)", "blur(1.4px)", "blur(4px)"] }} transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }} className="absolute -inset-[8%] h-[116%] w-[116%]">
      <defs>
...
export default function EnterpriseHero({ onOpenConsultation }: EnterpriseHeroProps) {
  return (
    <section className="relative min-h-[720px] overflow-hidden border-b border-white/10 [perspective:1200px]">
      <EnterpriseBackground />

      <div className="relative z-10 mx-auto w-full max-w-6xl px-4 pb-12 pt-28 text-center sm:px-6 lg:pt-36">
        <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.12 } } }}>
          <motion.h1 variants={fadeUp} transition={{ duration: 0.65 }} className="mx-auto max-w-5xl text-5xl font-semibold leading-[1.03] tracking-[-0.05em] text-white md:text-6xl lg:text-[5.4rem]">
            Innovation doesn&apos;t wait.<br /> Neither does <span className="text-sky-300">Skora.</span>
          </motion.h1>

          <motion.p variants={fadeUp} transition={{ duration: 0.6 }} className="mx-auto mt-6 max-w-2xl text-lg font-medium leading-relaxed text-slate-200 md:text-xl">
          ...
          <motion.div variants={fadeUp} transition={{ duration: 0.55 }} className="mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <button
              onClick={() => onOpenConsultation()}
              className="inline-flex items-center gap-2 rounded-lg bg-white px-6 py-3.5 text-sm font-bold text-[#071a42] shadow-lg shadow-black/20 transition hover:-translate-y-0.5 hover:bg-sky-200 cursor-pointer"
            >
              <Calendar size={17} /> Schedule Consultation <ArrowRight size={16} />
            </button>
            <a
              href="#capabilities"
              className="inline-flex items-center gap-2 rounded-lg border border-white/35 bg-white/10 px-6 py-3.5 text-sm font-bold text-white shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:border-white hover:bg-white/20"
            >
              <Layers size={17} /> Explore Capabilities
            </a>
          </motion.div>
        </motion.div>
          ...
          <div className="flex min-w-max items-stretch justify-center gap-3 px-2 lg:min-w-0 lg:flex-wrap">
            {profileCards.map((card, index) => (
              <a href={card.link} key={card.title}>
                <Card3D maxTilt={12} className="w-[174px] overflow-hidden rounded-xl border border-white/20 bg-white/10 p-2 text-left shadow-[0_14px_32px_rgba(0,0,0,0.16)] backdrop-blur-xl transition hover:border-sky-200/65 hover:bg-white/15">
                  <div className="h-24 overflow-hidden rounded-lg">
                    <img src={card.img} alt={card.title} className="h-full w-full object-cover transition duration-500 hover:scale-105" />
                  </div>
